    ...
    <>
      <nav className="fixed top-0 z-[70] w-full flex items-center justify-between px-6 py-6 glass border-b border-white/5 md:px-12 transition-all duration-500">
        ...
        <div className="hidden md:flex items-center gap-10">
          <Link href="/#how-it-works" className="text-[10px] font-black tracking-[0.2em] text-muted hover:text-accent transition-colors uppercase font-bold">Nasıl Çalışır?</Link>
          <Link href="/#restaurants" className="text-[10px] font-black tracking-[0.2em] text-muted hover:text-accent transition-colors uppercase font-bold">Mekanlar</Link>
          <Link href="/login" className="px-6 py-2.5 bg-accent text-black font-black rounded-2xl text-[10px] tracking-[0.1em] hover:shadow-[0_0_30px_rgba(245,158,11,0.3)] transition-all active:scale-95 uppercase">GİRİŞ YAP</Link>
        </div>
      ...
      <div className={`fixed inset-0 z-[65] bg-background/95 backdrop-blur-2xl transition-all duration-500 md:hidden ${isMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        ...
        <div className="relative flex flex-col items-center justify-center h-full gap-10 p-6 text-center">
          <Link 
            onClick={() => setIsMenuOpen(false)}
            href="/#how-it-works" 
            className="font-display text-4xl font-black tracking-tighter hover:text-accent transition-colors uppercase italic animate-in slide-in-from-bottom-4 duration-500 delay-150"
          >
            Nasıl Çalışır?
          </Link>
          <Link 
            onClick={() => setIsMenuOpen(false)}
            href="/#restaurants" 
            className="font-display text-4xl font-black tracking-tighter hover:text-accent transition-colors uppercase italic animate-in slide-in-from-bottom-4 duration-500 delay-200"
          >
            Mekanlar
          </Link>
          <Link 
            onClick={() => setIsMenuOpen(false)}
            href="/login" 
            className="w-full max-w-xs py-6 bg-accent text-black font-black rounded-[2rem] text-sm tracking-[0.3em] uppercase shadow-[0_0_50px_rgba(245,158,11,0.3)] animate-in slide-in-from-bottom-4 duration-500 delay-300"
          >
            GİRİŞ YAP
          </Link>
