                        ...
                        <Link key={cat.name} href={`/restaurants`} className="group relative">
                            <div className="relative aspect-[4/3] rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-card transition-all duration-700 hover:border-accent/30">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-8">
                                    <span className="text-5xl md:text-6xl mb-4 md:mb-6 group-hover:scale-125 transition-transform duration-500">{cat.icon}</span>
                                    <h3 className="font-display text-2xl md:text-5xl font-black text-white text-center leading-none tracking-tighter group-hover:text-accent transition-colors uppercase italic">{cat.name}</h3>
                                </div>
                            </div>
                        </Link>
                    ))}
